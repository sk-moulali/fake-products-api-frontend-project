...
export default function ProductsPage() {
  ...
  return (
    <>
      <h1 className="text-3xl mt-30 font-semibold  text-center">
        Explore Our Products
      </h1>
      <div className="mt-5 flex flex-row-reverse flex-wrap justify-center items-center gap-10">
        {products.map((product) => {
          const { id, image, category, title, price } = product;
          return (
            <div key={id}>
              <div className="w-80 m-auto">
                <div className="border border-[#e4e4e4] h-[300px] mb-1">
                  <div className="w-full h-full flex justify-center items-center shadow-lg">
                    {/* image */}
                    <div className="w-[200px] mx-auto flex justify-center items-center">
                      <Link to={`/product/${id}`}>
                        <img
                          className="max-h-[160px] cursor-pointer"
                          src={image}
                          alt=""
                        />
                      </Link>
